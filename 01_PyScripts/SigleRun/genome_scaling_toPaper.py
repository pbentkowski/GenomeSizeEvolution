...
Gene_max = 25.0
Res_max = 3.5
x = p.arange(0.0, Gene_max + 1.0, 1.0)
y = gene_cost * (x + x**2 + core_genes) + const_cost
Y_max = p.ceil(y.max() / 10.0) * Res_max
...
print "-----------------------------------------------------------"
print "Mean number of genes      :", repr_mean_number_of_genes
print "Total gain per gene       :", MEAN_tot_gain / repr_mean_number_of_genes
print "Mean net uptake per gene  :", MEAN_itake / repr_mean_number_of_genes
print "-----------------------------------------------------------"
...
p.title(r'$\delta = %(death)1.3f $    $T = %(turb)1.3f $ ' %
        {"death": death_rate, "turb": tubulence}, fontsize=FontSize + 2)
